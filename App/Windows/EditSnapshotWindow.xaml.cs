...
namespace App.Windows
{
    public sealed partial class EditSnapshotWindow : Window
    {
        ...
        private async Task CopyTextAndCloseAsync()
        {
            ...
            {
                await ShowCopyTextErrorAsync("An AI provider API key is required to copy text from a snapshot.");
                return;
            }
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(text)) throw new InvalidOperationException("The AI provider returned no text for the selected image.");
